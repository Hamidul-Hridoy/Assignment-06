...
const displayAll = (trees) => {
    ...
    for(let tree of trees){
        ...
        div2.innerHTML =`
        <div class="w-[343.33px] h-auto bg-white p-[16px]">
            ...
            <div class="flex justify-between mt-[8px]">
                <button class="w-[86px] h-[28px] rounded-[400px] bg-[#DCFCE7] text-[#15803D] text-[14px] font-semibold geist">${tree.category}</button>
                <h2 class="text-[14px] font-bold text-[#1F2937]"><i class="fa-solid fa-bangladeshi-taka-sign"></i>${tree.price}</h2>
            </div>
            <button class="w-[311.33px] h-[43px] text-left focus:h-[35px] pl-[10px] bg-[#15803D] font-semibold text-white text-[16px] rounded-[999px] px-[20px] py-[12px]">Add to Cart </button>
        </div>
